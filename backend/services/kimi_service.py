...
【系统可用内置应用模块 / 官方技能】
- 若用户的业务指令是处理含有建筑结构逻辑（如同层同行同列合并、包含叠合坐标表与线表的Excel文件夹等）并渲染3D效果的任务，请不要自行设计原生的Excel清洗及Plotly组装等底层步骤逻辑，这是毫无必要的；请直接安排使用本项目自带的专供SDK组件：`backend.sdk.excel_visualizer.Excel3DVisualizer`。
...
【系统全局可用 SDK 与内置技能指导】:
为了免除重复造轮子并在复杂抽象任务中降本，本项目为你内置了一系列 SDK，如果有涉及符合情况的环境需求，请通过生成导入该模块代码使用，绝不要生硬编写长篇重复的底层实现。
- 『Excel专属建筑模型的三维数据展现』：若是遇见含表类型（“叠合名表/状态表/平面线表等”）和网式骨架逻辑匹配业务的高阶需求场景，请调用专属组件，调用方法如下：
  1. 导包：因为你的代码实际是在独立隔离的系统临时沙箱目录中裸跑的，直接导包会报找不到模块，所以在导包的最前面必须硬编码寻址：
import sys
sys.path.append(r"d:\code\EasyCode")
from backend.sdk.excel_visualizer import Excel3DVisualizer
  2. 若要自动弹窗给甲方去用鼠标选定目标目录（极度推荐这一项方式）：直接实例并调取 `Excel3DVisualizer().run_gui()`
  3. 若要在后台代码里做无人职守跑通：`vis = Excel3DVisualizer(); vis.load_directory(文件路径变量); vis.merge_data(); vis.render_3d()`
